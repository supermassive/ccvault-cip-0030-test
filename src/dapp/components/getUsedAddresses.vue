...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("getUsedAddresses", "Check", [
    ...
    async function performCheck() {
      ...
      try {
        let r: any = await returnsPromise(
          logId,
          "getUsedAddresses",
          props.getUsedAddresses,
        );

        if (!isArray(r)) {
          return setApiTestFailed("getUsedAddresses: return type not an array");
        }
        ...
        if (ar.length > 1) {
          ...
          if (!isArray(r)) {
            return setApiTestFailed(
              "getUsedAddresses: return type not an array",
            );
          }
        ...
        addLogError(
          logId,
          "getUsedAddresses: error: " + JSON.stringify(e, null, 2),
        );
