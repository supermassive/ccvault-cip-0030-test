<template>
  ...
  <check-get-utxos-amount
    :log-id-suffix="logIdSuffix"
    :get-utxos="getUtxos"
    :utxo-amount="utxoAmount"
    v-if="getUtxos && utxoAmount && logIdSuffix === 'CIP'"
  />
</template>
...
import { decodeCborUtxo, logUtxo, toHexString } from "../lib/utilsCbor";
      ...
      try {
        ...
        for (const utxo of r) {
          ...
          if (utxoAmount.value === null) {
            ...
            utxoAmount.value = toHexString(
              decoded.output().amount().to_bytes(),
            );
          }
        }
      } catch (e: any) {
