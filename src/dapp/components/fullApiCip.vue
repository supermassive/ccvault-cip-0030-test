<template>
  ...
  <check-get-network-id
    log-id-suffix="CIP"
    :get-network-id="getNetworkIdFunction"
    v-if="getNetworkIdFunction"
  />
  <check-get-balance
    log-id-suffix="CIP"
    :get-balance="getBalanceFunction"
    v-if="getBalanceFunction"
  />
  <check-get-utxos
    log-id-suffix="CIP"
    :get-utxos="getUtxosFunction"
    v-if="getUtxosFunction"
  />

  <check-get-used-addresses
    log-id-suffix="CIP"
    :get-used-addresses="getUsedAddressesFunction"
    v-if="getUsedAddressesFunction"
    @onUsedAddress="onUsedAddress"
  />
  <check-get-unused-addresses
    log-id-suffix="CIP"
    :get-unused-addresses="getUnusedAddressesFunction"
    v-if="getUnusedAddressesFunction"
  />
  <!-- array of addresses -->
  <check-get-reward-addresses
    log-id-suffix="CIP"
    :get-reward-addresses="getRewardAddressesFunction"
    v-if="getRewardAddressesFunction"
  />
  <check-get-change-address
    log-id-suffix="CIP"
    :get-change-address="getChangeAddressFunction"
    v-if="getChangeAddressFunction"
  />

  <check-sign-data
    log-id-suffix="CIP"
    :sign-data="signDataFunction"
    v-if="signDataFunction && signDataAddress"
...
import { LogLevel, useApiLog } from "../useApiLog";
...
import { isBoolean, isObject } from "../lib/utils";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("Full API: CIP", "enable()", [
    ...
    async function performCheck() {
      ...
      if (typeof window.cardano[props.walletName] !== "object") {
        ...
        addLogSucceeded(logId, props.walletName + " object present.");
      }
      ...
      try {
        ...
        if (!isBoolean(r)) {
          return addLogError(logId, "isEnabled: return type not boolean");
        }
      ...
      try {
        ...
        if (!isObject(apiObj)) {
          return addLogError(logId, "enable: return type not object");
        }
        ...
        addLogError(
          logId,
          "enable: error: " + e + " code: " + e.code + " info: " + e.info,
        );
      }
