...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("getUnusedAddresses", "Check", [
    ...
    async function performCheck() {
      ...
      try {
        let r: any = await returnsPromise(
          logId,
          "getUnusedAddresses",
          props.getUnusedAddresses,
        );

        if (!isArray(r)) {
          return setApiTestFailed(
            "getUnusedAddresses: return type not an array",
          );
        }
        ...
        addLogError(
          logId,
          "getUnusedAddresses: error: " + JSON.stringify(e, null, 2),
        );
