...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("getRewardAddresses", "Check", [
    ...
    async function performCheck() {
      ...
      try {
        let r: any = await returnsPromise(
          logId,
          "getRewardAddresses",
          props.getRewardAddresses,
        );

        if (!isArray(r)) {
          return setApiTestFailed(
            "getRewardAddresses: return type not an array",
          );
        }
        ...
        addLogError(
          logId,
          "getRewardAddresses: error: " + JSON.stringify(e, null, 2),
        );
