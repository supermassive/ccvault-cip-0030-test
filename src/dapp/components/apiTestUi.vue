<template>
  ...
  >
    ...
    <div class="col-span-12 grid grid-cols-2 gap-1 h-6 text-xs">
      <span
        class="px-2 font-semibold rounded-full flex justify-center items-center"
        :class="apiTest.statusCSS"
        >{{ apiTest.status }}</span
      >
      <button
        class="px-2 font-semibold rounded-full flex justify-center items-center text-indigo-600 hover:text-indigo-900 bg-gray-200"
        @click="performCheck"
      >
        {{ apiTest.buttonLabel }}
      </button>
    ...
    <div class="col-span-12 grid grid-cols-2 gap-1 h-6 text-xs">
      ...
      <button
        class="px-2 font-semibold text-indigo-600 hover:text-indigo-900 rounded-full bg-gray-200"
        @click="clearLogs"
      >
        Clear Logs
      </button>
    ...
    <div
      class="col-span-12 h-60 border border-gray-200 rounded-xl p-1.5 inline-flex flex-col flex-nowrap text-left text-sm max-h-24 overflow-auto"
    >
      <div
...
export default defineComponent({
  ...
  setup(props, context) {
    const hasSlotButtons = computed(() =>
      context.slots.hasOwnProperty("buttons"),
    );
    const hasSlotContent = computed(() =>
      context.slots.hasOwnProperty("content"),
    );
