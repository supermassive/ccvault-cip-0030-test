<template>
  ...
  />
  <!-- single address -->
  <check-get-reward-address
    log-id-suffix="legacy"
    :get-reward-address="getRewardAddressFunction"
    v-if="getRewardAddressFunction"
  />
...
export default defineComponent({
  ...
  setup() {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("Full API: Legacy", "enable()", [
    ...
    async function performCheck() {
      ...
      try {
        ...
        if (!isObject(r) && !isBoolean(r)) {
          return addLogError(
            logId,
            "enable: return type not object or boolean",
          );
        }
