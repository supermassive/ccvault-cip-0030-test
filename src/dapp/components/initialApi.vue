<template>
  <div class="col-span-12 grid grid-cols-12 gap-2">
    <button
      class="col-span-6 md:col-span-4 xl:col-span-3 2xl:col-span-2 px-2 h-6 text-sm font-semibold text-indigo-600 hover:text-indigo-900 rounded-full bg-gray-200"
      @click="$emit('connect', 'legacy')"
      v-if="showConnectLegacy"
    >
      Connect: Legacy Api
    </button>
    <button
      class="col-span-6 md:col-span-4 xl:col-span-3 2xl:col-span-2 px-2 h-6 text-sm font-semibold text-indigo-600 hover:text-indigo-900 rounded-full bg-gray-200"
      @click="$emit('connect', name)"
      v-for="name in walletNames"
      :key="name"
    >
      Connect CIP-0030: {{ name }}
    </button>
  </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("Initial API Functions", "Detect APIs", [
    ...
    const showConnectButtons = computed(
      () => apiTest.status === ApiTestStatus.succeeded,
    );
    ...
    function performCheck() {
      ...
      if (typeof window === "undefined") {
        return setApiTestFailed("No window object present.");
      } else {
        addLogSucceeded(logId, "window object present.");
      }

      // @ts-ignore
      if (typeof window.cardano !== "object") {
        return setApiTestFailed("No cardano object present.");
      } else {
        addLogSucceeded(logId, "cardano object present.");
      }
      ...
      if (cardano.hasOwnProperty("isEnabled")) {
        ...
        if (!checkFunctionsExists(logId, cardano, legacyProps)) {
          return setApiTestFailed("Legacy api is broken.");
        } else {
          addLogSucceeded(logId, "Legacy api available.");
        }
      ...
      for (const walletName in cardano) {
        ...
        if (
          !checkFunctionsExists(logId, cardano[walletName], initialApiFuncs) ||
          ...
          addLogSucceeded(logId, "CIP-0030 initial api available.");
        }
      ...
      for (const walletName of walletNames.value) {
        ...
        addLogImportant(
          logId,
          'icon: <img src="' + cardano[walletName].icon + '">',
        );
      }
