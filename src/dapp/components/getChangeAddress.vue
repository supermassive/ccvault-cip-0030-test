...
import { isArray, isString, returnsPromise } from "../lib/utils";

import { addApiTest } from "../lib/ApiTestSuite";

import { decodeCborAddress, logAddress } from "../lib/utilsCbor";
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("getChangeAddress", "Check", [
    ...
    async function performCheck() {
      ...
      try {
        let r: any = await returnsPromise(
          logId,
          "getChangeAddress",
          props.getChangeAddress,
        );

        if (!isString(r)) {
          return setApiTestFailed(
            "getChangeAddress: return type not an string",
          );
        }
        ...
        addLogError(
          logId,
          "getChangeAddress: error: " + JSON.stringify(e, null, 2),
        );
