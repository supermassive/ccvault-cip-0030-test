...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apiTest: ApiTest = reactive<ApiTest>(
      createApiTest("getRewardAddress", "Check", [
    ...
    async function performCheck() {
      ...
      try {
        let r: any = await returnsPromise(
          logId,
          "getRewardAddress",
          props.getRewardAddress,
        );

        if (!isString(r)) {
          return setApiTestFailed("getRewardAddress: return type not an array");
        }
        ...
        addLogError(
          logId,
          "getRewardAddress: error: " + JSON.stringify(e, null, 2),
        );
